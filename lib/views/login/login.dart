...
class _LoginState extends State<Login> with TickerProviderStateMixin {
      ...
      child: WillPopScope(
        onWillPop: () async {
          AppNavigator.pushReplace(
              context: context, page: Welcome(verified: widget.verified));
          return true;
        },
        child: Scaffold(
                    ...
                    children: <Widget>[
                      ...
                      AppTextField(
                        ...
                        onValidate: (value) {
                          if (value.isEmpty) {
                            return AppLocalizations.of(context)
                                .translate('password_empty_validate');
                          }

                          if (value.length < 3) {
                            return AppLocalizations.of(context)
                                .translate('password_len_validate');
                          }
  ...
  void submitForm() async {
    ...
    if (_formKey.currentState.validate()) {
      ...
      try {
        result = await Provider.of<FirebaseAuthService>(context, listen: false)
  ...
  void getData(UserModel user) async {
    try {
      if (user.uID != null) {
        ...
        if (result is UserModel) {
          dynamic resultCompany =
              await Provider.of<FirestoreService>(context, listen: false)
                  .getCompanyData(uID: result.companyID);
          if (resultCompany is UserModel) {
            Provider.of<CompanyViewModel>(context, listen: false)
                .setUserModel(resultCompany);
            if (result.status) {
              if (result.roleID == AppConfig.internUserRole.toDouble()) {
                ...
                if (result.verified) {
                  await Provider.of<InternsViewModel>(context, listen: false)
                      .fetchData(uID: resultCompany.uID);
                ...
                Message.show(_globalScaffoldKey,
                    AppLocalizations.of(context).translate('login_permission'));
              }
            } else {
              setState(() {
                emailSt = true;
              ...
              Message.show(
                  ...
                  AppLocalizations.of(context)
                      .translate('suspended_account_message'));
            }
          } else {
            setState(() {
              emailSt = true;
            ...
            _buttonAnimationController.reverse();
            Message.show(_globalScaffoldKey,
                AppLocalizations.of(context).translate('no_company_linked'));
          }
        } else {
          setState(() {
            emailSt = true;
          ...
          Message.show(
              _globalScaffoldKey,
              AppLocalizations.of(context)
                  .translate('receive_error_description'));
        }
      }
    } catch (error) {
      if (!AppConfig.isPublished) {
      ...
      _buttonAnimationController.reverse();
      Message.show(_globalScaffoldKey,
          AppLocalizations.of(context).translate('receive_error_description'));
    }
  }
