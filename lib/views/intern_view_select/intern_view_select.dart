...
class _InternViewSelectState extends State<InternViewSelect> {
  ...
  void performDelete() async {
    ...
    dynamic result = await Provider.of<FirestoreService>(context, listen: false)
        .deleteUser(uID: widget.intern.uID);
    if (result is bool && result) {
      Provider.of<InternsViewModel>(context, listen: false)
          .removeFromList(model: widget.intern);
      await Future.delayed(Duration(milliseconds: 1500), () {
        Navigator.pop(context);
      });
      await Future.delayed(Duration(milliseconds: 800), () {
        Message.show(_globalScaffoldKey,
            AppLocalizations.of(context).translate('delete_intern_success'));
      });
      await Future.delayed(Duration(milliseconds: 1500), () {
        Navigator.pop(context);
      });
      ...
      await Future.delayed(Duration(milliseconds: 800), () {
        Message.show(_globalScaffoldKey,
            AppLocalizations.of(context).translate('delete_intern_fail'));
      });
    }
  }
  ...
  Widget internViewSelectBody() {
    return Column(
      ...
      children: [
        ...
        SizedBox(height: 30),
        Text(AppLocalizations.of(context).translate('what_to_view'),
        ...
        SizedBox(
          ...
          child: OutlinedButton(
              onPressed: () async {
                ...
                AppNavigator.push(
                    context: context,
                    page: ClocksDetail(intern: widget.intern));
              },
              child: Text(
                  AppLocalizations.of(context).translate('clock_in_out'),
                  style: TextStyle(
                      color: AppColors.primaryColor,
                      fontSize: 15,
          ...
          child: OutlinedButton(
              onPressed: () async {
                ...
                AppNavigator.push(
                    context: context,
                    page:
                        ContactInfo(userModel: widget.intern, isIntern: true));
              },
              child: Text(
